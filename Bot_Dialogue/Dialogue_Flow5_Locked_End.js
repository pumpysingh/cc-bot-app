    ...
    var nlpcallbacks = function (session, message) {
        return {
            ...
            match: function (response) {
                var keys = [];
                var values = [];
                keys.push("Flow5_Locked_End");
                values.push(response.result.metadata.intentName);
                if (response.result.metadata.intentName == "YesReturn") {
                    utility.setConversationDatas(session, keys, values, true, "Flow5_Locked_End_Yes", response.result.metadata.intentName);
                    utility.StoreErrorLogs(constant.matched, 'Flow5_Locked_End', session.conversationData.conversationId);
                    session.beginDialog("Flow2");
                }
                else if (response.result.metadata.intentName == "NoEnd") {
                    utility.setConversationDatas(session, keys, values, true, "Flow5_Locked_ChatEnd", response.result.metadata.intentName);
                    utility.StoreErrorLogs(constant.matched, 'Flow5_Locked_End', session.conversationData.conversationId);
            ...
            repromptquestion: function (response) {
                ...
                utility.StoreErrorLogs(message, 'Flow5_Locked_End', session.conversationData.conversationId);
                session.replaceDialog('Flow5_Locked_End', { data: session.conversationData, reprompt: true });
            },
            tooLargeMessage: function (response) {
                session.conversationData.lastDialogName = 'Flow5_Locked_End';
                utility.StoreErrorLogs(message, 'Flow5_Locked_End', session.conversationData.conversationId);
                session.replaceDialog('Flow5_Locked_End', { data: session.conversationData, toolargereprompt: true });
            }
        }
    }
}